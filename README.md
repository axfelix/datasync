...
Includes Linux, Mac, and Windows installers. Most of the guts are this are just bash, and I like it that way. Platform-specific notes are as follows:
...
Windows version should work on XP or newer (tested on XP SP3 and Win7 x64). Windows version uses NSIS (and the nsUnzip plugin; nsis.sourceforge.net, not included) to deploy a prepared Cygwin installation and run configuration scripts in a single .exe. It's much larger due to the Cygwin dependencies, but it uses the same underlying cron/unison guts as the other platforms. This was by far the hardest to create, so I hope you appreciate it most.
