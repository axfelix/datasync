...
echo -e 'if ping -n 1 datadisk.lib.sfu.ca | grep "TTL" ; then\nif ps -W | grep "~[/].datastage"; then\nexit\nelse\nunison.exe ~/Documents/datadisk ssh://'$user'@researchdata.sfu.ca:/home/'$user'/.pydiodata -batch -backups -copythreshold 5000 -prefer root -ignore="Name *.tmp" -ignore="Name *~"\nfi\nfi' > ~/.datastage.sh