...
then
	...
	echo -e 'if ps -W | grep "~[/].datastage"; then\nexit\nelse\nunison.exe "C:\Documents and Settings\'$USERNAME'\datadisk" ssh://'$user'@researchdata.sfu.ca//home/'$user'/.pydiodata -batch -backups -prefer "C:\Documents and Settings\'$USERNAME'\datadisk" -ignore="Name *.tmp" -ignore="Name *~"\nfi' > "/cygdrive/c/Documents and Settings/$USERNAME/.datastage.sh"
	...
	echo -e 'if ps -W | grep "~[/].datastage"; then\nexit\nelse\nunison.exe C:\Users\'$USERNAME'\datadisk ssh://'$user'@researchdata.sfu.ca//home/'$user'/.pydiodata -batch -backups -prefer C:\Users\'$USERNAME'\datadisk -ignore="Name *.tmp" -ignore="Name *~"\nfi' > /cygdrive/c/Users/$USERNAME/.datastage.sh
